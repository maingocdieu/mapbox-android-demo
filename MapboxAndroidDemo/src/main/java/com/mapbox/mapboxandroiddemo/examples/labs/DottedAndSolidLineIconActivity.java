...
/**
 * Combine different styled lines with a SymbolLayer icon(s)
 */
public class DottedAndSolidLineIconActivity extends AppCompatActivity {

  private MapView mapView;

  private Point newarkAirport = Point.fromLngLat(
    -74.17799, 40.69297);
  private Point sanFranciscoAirport = Point.fromLngLat(
    -122.38709, 37.616714);
  private Point minneapolisCityAirport = Point.fromLngLat(
    -93.224, 44.8815);
      ...
      public void onMapReady(@NonNull MapboxMap mapboxMap) {
        mapboxMap.setStyle(Style.SATELLITE_STREETS, new Style.OnStyleLoaded() {
          ...
          public void onStyleLoaded(@NonNull Style style) {
            ...
            style.addSource(new GeoJsonSource("plane-source-id", minneapolisCityAirport));
            ...
            // Add the marker icons which represent the plane's origin and destination
            style.addImage("destination-and-origin-icon-id", BitmapFactory.decodeResource(
              getResources(), R.drawable.red_marker));
            style.addSource(new GeoJsonSource("destination-and-origin-source-id", FeatureCollection.fromFeatures(
              new Feature[] {
                Feature.fromGeometry(newarkAirport), Feature.fromGeometry(sanFranciscoAirport)
              })));
            ...
            dotList.add(newarkAirport);
            dotList.add(minneapolisCityAirport);
            ...
            solidLineList.add(minneapolisCityAirport);
            solidLineList.add(sanFranciscoAirport);
